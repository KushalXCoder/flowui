...
<Card
    title="Note"
    description="Currently, this component just provides a structured UI. Logic is required to be handled by you."
    className="mb-8"
/>
...
Run the following command to add the `file-select` component to your project using the Flow UI CLI:

```bash
npx theflowui add file-select
```
